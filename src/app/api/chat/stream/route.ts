...
const SYSTEM_PROMPT = `Ты — не ассистент и не ИИ.
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isPro) {
      ...
      if (count && count >= 10) {
        return new Response(
          JSON.stringify({ error: 'Лимит бесплатных сообщений исчерпан. Обновите тариф до Pro.' }),
      ...
      if (sessions && sessions.length > 0) {
        ...
        if (count && count >= 500) {
          return new Response(
            JSON.stringify({ error: 'Лимит Pro сообщений исчерпан (500/мес). Обновите подписку для продолжения.' }),
            { status: 403, headers: { 'Content-Type': 'application/json' } }
          );
        }
      }
